...
lon = ds['lon'][:].values
lat = ds['lat'][:].values
depth = ds['elevation'][:].values  # 変数名はファイルによって違うかも

# GEBCOデータの範囲
lat_min_src, lat_max_src = lat.min(), lat.max()
lon_min_src, lon_max_src = lon.min(), lon.max()

# 2. 格子範囲・解像度のインタラクティブ指定
# ここはinput()で例示（JupyterならwidgetsもOK）
print(f"GEBCOデータの緯度範囲: {lat_min_src} ~ {lat_max_src}")
print(f"GEBCOデータの経度範囲: {lon_min_src} ~ {lon_max_src}")
lon_min = float(input("格子西端経度："))
lon_max = float(input("格子東端経度："))
lat_min = float(input("格子南端緯度："))
lat_max = float(input("格子北端緯度："))
d_lon = float(input("経度間隔（度）："))
d_lat = float(input("緯度間隔（度）："))

# 入力値をガード（外れそうなら警告）
if not (lat_min_src <= lat_min <= lat_max_src and lat_min_src <= lat_max <= lat_max_src):
    raise ValueError(f"格子の緯度範囲は{lat_min_src}～{lat_max_src}の間で指定してください")
if not (lon_min_src <= lon_min <= lon_max_src and lon_min_src <= lon_max <= lon_max_src):
    raise ValueError(f"格子の経度範囲は{lon_min_src}～{lon_max_src}の間で指定してください")
...
# 4. GEBCOデータから内挿
interp = RegularGridInterpolator((lat[::-1], lon), depth[::-1, :])  # lat, lon順に注意
...
bathy_grid = np.nan_to_num(bathy_grid, nan=0)  # 万一nanが混じったら0で埋める

# 5. 可視化（任意）
# 5.1. マスク作成（0を特別扱い）
masked_bathy = np.ma.masked_where(bathy_grid == 0, bathy_grid)

# 5.2. 元のカラーマップ取得・0用に白を挿入
base_cmap = plt.get_cmap('viridis')  # お好きなcolormapでOK
colors = base_cmap(np.linspace(0, 1, 256))
cmap_with_white = ListedColormap(np.vstack(([1, 1, 1, 1], colors)))  # 先頭に白を追加

# 5.3. 正の水深値の最大値を取得（カラーマップの範囲用）
...
with open(grd_file, "w") as f:
    # 経度：先に「行方向に」1次元にflatten
    for val in grid_lon2d.flatten(order='C'):  # 行優先(C)でフラット化
        f.write(f"{val:.8f}\n")
    # 緯度も同様に
    for val in grid_lat2d.flatten(order='C'):
        f.write(f"{val:.8f}\n")

print('グリッドデータを書き出しました．')
